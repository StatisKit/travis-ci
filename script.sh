...
if [[ ! "$CONDA_RECIPE" = "" ]]; then
  ...
  $TRAVIS_WAIT $SUDO_CMD docker build --build-arg CONDA_VERSION=${CONDA_VERSION} -t ${DOCKER_UPLOAD}/${DOCKER_CONTAINER}:${TRAVIS_TAG}-py${CONDA_VERSION}k ${DOCKER_CONTAINER}
